...
vec3 uchimuraF(vec3 x, float P, float a, float m, float l, float c, float b) {
  float l0 = ((P - m) * l) / a;
  float L0 = m - m / a;
  float L1 = m + (1.0 - m) / a;
  float S0 = m + l0;
...
vec3 uchimura(vec3 x) {
  ...
  return uchimuraF(x, P, a, m, l, c, b);
}
...
vec3 agxCustom(vec3 color) {
  return agxCdl(color, vec3(1.0, 1.0, 1.0), vec3(0.0), vec3(1.1), 1.1);
}
...
function configurePipeline(pipeline) {
  const mainTexture = pipeline.createTexture("mainTexture").format(Format.RGBA16F).width(screenWidth).height(screenHeight).clear(true).build();
  ...
  const bloomTex = pipeline.createTexture("bloomTex").format(Format.RGBA16F).width(screenWidth).height(screenHeight).clear(true).mipmap(true).build();
  ...
  postRender.createComposite("lighting").vertex("post/fullscreen_Pass.vsh").fragment("post/lighting.fsh").target(0, mainTexture).target(1, bloomTex).compile();
  for (let i = 0; i < 5; i++) {

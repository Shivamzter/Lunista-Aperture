...
function configurePipeline(pipeline) {
  let mainTexture = pipeline.createTexture("mainTexture").width(screenWidth).height(screenHeight).format(Format.RGB16F).build();
  ...
  pipeline.createObjectShader("basic", Usage.BASIC).location("objects/basic").exportBool("disableFog", true).target(0, mainTexture).target(1, lightmapTex).target(2, normalTexture).target(3, flatNormalTex).target(4, labSpecularTex).blendOff(3).compile();
